...
export default function Home() {
  ...
  function finishRoadEditing(){setEditingRoads(false);setActiveRoadPathId(null);setSelectedRoadPoint(null);setMovingRoadPoint(false);setShowRoadEditorLegend(false);setShowGpsCalibration(false);roadUndoStackRef.current=[];setRoadUndoCount(0);setShowLayers(false);}
    ...
    <main className="delivery-app">
      ...
      <section className={`main-layout view-${activeTab}`}>
        ...
        <section className={`map-card ${editingRoads?"road-editing":""}`}>
          ...
          {editingRoads&&<div className="road-editor-tools"><button className={showRoadEditorLegend?"active":""} onClick={()=>{setShowRoadEditorLegend((value)=>!value);setShowGpsCalibration(false);}}>Key</button><button className={showGpsCalibration?"active":""} onClick={()=>{setShowGpsCalibration((value)=>!value);setShowRoadEditorLegend(false);}}>GPS <span>{gpsCalibration.count}</span></button></div>}
