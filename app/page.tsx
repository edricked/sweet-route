    ...
    <main className="delivery-app">
      ...
      <section className={`main-layout view-${activeTab}`}>
        ...
        {activeTab === "addresses" && <section className="settings-overlay address-book">
          ...
          {owner&&<div className="owner-home-card"><button className="owner-row" onClick={beginOwnerSetup}><span>⌂</span><div><strong>Owner home</strong><small>{addressLabel(owner)}{owner.gps?` · GPS ±${Math.round(owner.gps.accuracy)} m`:" · GPS not set"}</small></div><b>›</b></button><div className="owner-gps-actions"><button disabled={capturingOwnerGps} onClick={()=>void captureOwnerGps()}>{capturingOwnerGps?"Getting location…":owner.gps?"Update location":"Save current location"}</button>{owner.gps&&<button className="danger" onClick={()=>{if(window.confirm("Remove the home GPS coordinate?")){removeAddressGps(owner.id);setOwnerGpsMessage("Home GPS coordinate removed.");}}}>Remove</button>}</div>{ownerGpsMessage&&<p role="status">{ownerGpsMessage}</p>}</div>}
