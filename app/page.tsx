    ...
    <main className="delivery-app">
      ...
      <section className={`main-layout view-${activeTab}`}>
        ...
        <section className={`map-card ${editingRoads?"road-editing":""}`}>
          ...
          <div className={`map-action-stack ${editingRoads?"road-editor-zoom":""}`}><button aria-label="Zoom in" onClick={() => setZoom((value) => Math.min(MAX_MAP_ZOOM, value + .25))}>+</button><button aria-label="Zoom out" onClick={() => setZoom((value) => Math.max(minimumZoom, value - .25))}>−</button>{editingRoads?<button aria-label="Reset map view" onClick={resetMapView}>↺</button>:<button aria-label="Center on owner home" disabled={!owner} onClick={recenterOwner}>⌂</button>}</div>
